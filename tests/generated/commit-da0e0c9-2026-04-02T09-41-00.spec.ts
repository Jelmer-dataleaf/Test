...
test.describe('App header navigation', () => {
  ...
  test('renders the app header, logo, and all navigation links including newsletter', async ({ page }) => {
  ...
  test('newsletter navigation link is accessible and points to the newsletter route', async ({ page }) => {
  ...
  test('navigates to newsletter page when newsletter link is clicked', async ({ page }) => {
    await page.getByTestId('nav-newsletter').click();
    await expect(page).toHaveURL(/\/newsletter$/);
  });

  test('router outlet is present for route content rendering', async ({ page }) => {
    await expect(page.getByTestId('router-outlet')).toBeAttached();
  });

  test('navigation contains exactly seven links to prevent missing or duplicate entries', async ({ page }) => {
    const navLinks = page.locator('header nav a');
    await expect(navLinks).toHaveCount(7);
  });

  test('newsletter link text is normalized despite multiline HTML formatting', async ({ page }) => {
    const newsletterLink = page.getByTestId('nav-newsletter');
